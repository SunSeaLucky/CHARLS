...
missing_rate = 0.3
# 最终输出结果的文件名
final_file_name = 'D:\Personal\Program\MICE\Result3\Results\Res_' + str(missing_rate) + '.csv'
# 是否对最终结果进行模型插补
imputate = 1
...
fra = pd.concat([fra, cog], axis=1)
fra = CommonFunction.del_specific(fra, more_than_value)
if imputate and missing_rate != 0:
    fra = CommonFunction.imputation(fra)
