...
class DataOperator:
    def __init__(self, year: int, scale: str, source_df):
        ...
        #     'dc022',
        #     'dc023',
        #     'dc027s1', 'dc027s2', 'dc027s3',
        #     'dc027s4', 'dc027s5', 'dc027s6',
        #     'dc027s7', 'dc027s8', 'dc027s9',
        #     'da007_1_',
        #     'da007_2_',
        #     'da007_3_',
        #     'da007_4_',
        ...
        # ]
        #
        # self.indicators_2018_new_MMSE = []
        # self.indicators_2018_MMSE = [
        #     'dc001_w4',
        #     'dc002_w4',
        #     'dc003_w4',
        #     'dc005_w4',
        ...
        #     'dc023_w4',
        #     'dc024_w4',
        #     'ba000_w2_3'  # highest_level_of_education_attained
        # ]
        #
        # if year == 2011:
        #     if scale == 'PFP':
        #         self.indicators = self.indicators_2011_PFP
        #     elif scale == 'MMSE':
        #         self.indicators = self.indicators_2011_MMSE
        # elif year == 2013:
        #     if scale == 'PFP':
        #         self.indicators = self.indicators_2013_PFP
        #     elif scale == 'MMSE':
        #         self.indicators = self.indicators_2013_MMSE
        # elif year == 2015:
        #     if scale == 'PFP':
        #         self.indicators = self.indicators_2015_PFP
        #     elif scale == 'MMSE':
        #         self.indicators = self.indicators_2015_MMSE
        # elif year == 2018:
        #     if scale == 'new_MMSE':
        #         self.indicators = self.indicators_2018_new_MMSE
        #     elif scale == 'MMSE':
        #         self.indicators = self.indicators_2018_MMSE
    ...
    def calculate_PFP_indicators(self):
        ...
        for i in self.df.index.values:
            ...
            # 1.体重减轻
            BMI = float(self.df.loc[i, indicators[0]]) / ((float(self.df.loc[i, indicators[0]]) / 100) ** 2)
            score += 1 if BMI <= 18.5 else 0
            # 2.握力退减
            avg = (float(self.df.loc[i, indicators[2]]) + float(self.df.loc[i, indicators[4]]) + float(
                self.df.loc[i, indicators[3]]) + float(self.df.loc[i, indicators[5]])) / 4
            item_score = 0
            # 男性
            ...
            # 男性
            if self.df.loc[i, indicators[13]] == 1:
                if float(self.df.loc[i, indicators[1]]) <= 163 and float(self.df.loc[i, indicators[1]]) / 2.5 >= 0.45:
                    item_score = 1
                elif float(self.df.loc[i, indicators[1]]) > 163 and float(self.df.loc[i, indicators[1]]) / 2.5 > 0.48:
                    item_score = 1
            # 女性
            elif self.df.loc[i, indicators[13]] == 2:
                if float(self.df.loc[i, indicators[1]]) <= 151 and float(self.df.loc[i, indicators[1]]) / 2.5 >= 0.36:
                    item_score = 1
                elif float(self.df.loc[i, indicators[1]]) > 151 and float(self.df.loc[i, indicators[1]]) / 2.5 > 0.43:
                    item_score = 1
            score += item_score

            # 4.体力活动水平减弱
            if self.df.loc[i, indicators[8]] == 1 \
                    or self.df.loc[i, indicators[9]] == 1 \
                    or self.df.loc[i, indicators[10]] == 1:
                score += 1

            # 5.自报疲劳
            if float(self.df.loc[i, indicators[11]]) + float(self.df.loc[i, indicators[12]]) - 2 >= 3:
                score += 1
    ...
    def calculate_MMSE_indicators(self):
        ...
        for i in self.df.index.values:
            ...
            # 11
            score += 1 if int(self.df.loc[i, indicators[6]]) == 1 \
                          or int(self.df.loc[i, indicators[7]]) == 2 \
                          or int(self.df.loc[i, indicators[8]]) == 3 else 0
            # 12
            score += 1 if int(self.df.loc[i, indicators[9]]) == 4 \
                          or int(self.df.loc[i, indicators[10]]) == 5 \
                          or int(self.df.loc[i, indicators[11]]) == 6 else 0
            # 13
            score += 1 if int(self.df.loc[i, indicators[12]]) == 7 \
                          or int(self.df.loc[i, indicators[13]]) == 8 \
                          or int(self.df.loc[i, indicators[14]]) == 9 else 0
            # 14
            ...
            score += 1 if int(self.df.loc[i, indicators[6]]) == 1 \
                          ...
                          and int(self.df.loc[i, indicators[8]]) == 3 else 0
            # 20
            score += 1 if int(self.df.loc[i, indicators[9]]) == 4 \
                          and int(self.df.loc[i, indicators[10]]) == 5 \
                          and int(self.df.loc[i, indicators[11]]) == 6 else 0
            # 21
            score += 1 if int(self.df.loc[i, indicators[12]]) == 7 \
                          and int(self.df.loc[i, indicators[13]]) == 8 \
                          and int(self.df.loc[i, indicators[14]]) == 9 else 0
            ...
            for j in range(1, 15):
                if int(self.df.loc[i, indicators[19 + j]]) == 1:
                    tmp = True
                    break
            if int(self.df.loc[i, indicators[34]]) == 1 \
            ...
            cognition_array.append(1 if is_cog_imp(score, int(self.df.loc[i, indicators[75]])) else 0)
    ...
    def __calculate_MMSE_2018(self):
        ...
        for i in self.df.index.values:
            ...
            for j in range(14, 19):
                score += 1 if int(self.df.loc[i, indicators[j - 1]]) == (100 - 7 * (j - 13)) else 0
            # 19~21
            ...
            score_array.append(score)
            cognition_array.append(1 if is_cog_imp(score, self.df.loc[i, indicators[30]]) else 0)
        self.df['cognition_impairment'] = cognition_array
        self.df['score'] = score_array
